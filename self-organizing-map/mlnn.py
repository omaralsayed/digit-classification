...
class MLNN:
    def __init__(self, layers, neurons, epoch=0, epochs=5000, learning_rate=0.3, min_learning_rate=0.0025, momentum=0.9):
    ...
    def initialize_dictionary(self):
        ...
        hyperparameters['W1'] = np.loadtxt('som.txt').reshape(hidden[0], input_layer) * np.sqrt(1. / hidden[0])
    ...
    def backward_pass(self, y_train, output):
        ...
        for x in range(self.layers, 1, -1): # Updated from 0 to 1 to not update input weights
    ...
    def show_plots(self, accuracies, errors, x_train, y_train, x_test, y_test):
        ...
        plt.title('Balanced Accuracy Over 5000 Epochs')
        ...
        plt.title('Training Error Over 5000 Epochs')
        plt.ylabel('Error')
        plt.xlabel('Epochs')
